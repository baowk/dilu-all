	...
	if config.Get().RdConfig.Enable {
		...
		for _, v := range rdcfg.Registers {
			if v.Protocol != "grpc" && v.Protocol != "http" {
				core.GetApp().GetLogger().Error("rd register error", "protocol", v.Protocol)
				continue
			}
			if v.Protocol == "grpc" && !config.Get().GrpcServer.Enable {
				core.GetApp().GetLogger().Error("rd register error", "protocol", v.Protocol, "GrpcServer enable", false)
				continue
			}
			if v.Name == "" {
				if v.Protocol == "http" {
		...
		core.GetApp().GetLogger().Debug("注册中心连接", "rdcfg", rdcfg)
		var err error
